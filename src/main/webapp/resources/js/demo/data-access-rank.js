...
function getDataList() {
	if ($.fn.DataTable.isDataTable("#r1_dataTable"))
		$("#r1_dataTable").DataTable().clear().destroy();
	if ($.fn.DataTable.isDataTable("#r2_dataTable"))
		$("#r2_dataTable").DataTable().clear().destroy();
	if ($.fn.DataTable.isDataTable("#r3_dataTable"))
		$("#r3_dataTable").DataTable().clear().destroy();

	let s1 = $("#datetxt").val();
	let s2 = new Date((new Date(s1)).valueOf() - 1000 * 60 * 60 * 24 * 1)
			.toISOString().split('T')[0];
	let s3 = new Date((new Date(s1)).valueOf() - 1000 * 60 * 60 * 24 * 2)
			.toISOString().split('T')[0];
											...
											columns : [
													...
													},
													{
														...
														"fnCreatedCell" : function(
															...
															$(nTd)
																	.html(
																			"<a href=javascript:getStatis('"
																					+ encodeURIComponent(oData.keyword)
																					+ "');>"
																					+ oData.keyword
																					+ "</a>");
														}
